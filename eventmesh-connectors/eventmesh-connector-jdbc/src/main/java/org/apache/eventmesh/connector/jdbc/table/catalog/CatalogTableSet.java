...
public final class CatalogTableSet {
    ...
    public static TableEditor ofCatalogTableEditor(TableId tableId) {
        return new DefaultTableEditorImpl(tableId);
    }
    ...
    private static class TableIdSet {
        ...
        public void removeDatabase(String catalogName, String schemaName) {
            values.removeIf(entry -> {
                return StringUtils.equals(entry.getCatalogName(), catalogName) && StringUtils.equals(entry.getSchemaName(), schemaName);
            });
        }
    }
