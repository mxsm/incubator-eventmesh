...
public class MysqlSnapshotEngine extends
    ...
    protected void readStructureOfTables(MysqlJdbcContext jdbcContext, SnapshotContext<MysqlPartition, MysqlOffsetContext> snapshotContext)
        ...
        for (TableId tableId : determineTables) {
            ...
            addParseDdlAndEvent(jdbcContext, dropTableDdl.toString());
        }
        ...
        for (String database : databaseSet) {
            ...
            addParseDdlAndEvent(jdbcContext, dropDatabaseDdl.toString());
            ...
            addParseDdlAndEvent(jdbcContext, databaseCreateDdl);
            addParseDdlAndEvent(jdbcContext, "USE " + database);
    ...
    private void addParseDdlAndEvent(MysqlJdbcContext jdbcContext, String ddl) {
        ...
        for (TableId tableId : tableIds) {
            connection.query(MysqlDialectSql.SHOW_CREATE_TABLE.ofWrapperSQL(tableId.toString()), resultSet -> {
                if (resultSet.next()) {
                    ...
                    addParseDdlAndEvent(jdbcContext, createTableDdl);
                }
            });
        }
    }
